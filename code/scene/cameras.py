...
class Camera(nn.Module):
    def __init__(
        ...
        if image_norImg is not None:
            # print(f"==>> image_norImg: {type(image_norImg)}")
            # g(image_norImg)
            self.original_image_norImg = image_norImg.clamp(0.0, 1.0).to(self.data_device)

        else:
            self.original_image_norImg = torch.zeros((3, self.image_height, self.image_width))
