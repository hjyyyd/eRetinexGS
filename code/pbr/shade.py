...
def pbr_shading(
    ...
    view_dirs = view_dirs.reshape(1, H, W, 3)   # 不太确定是 世界坐标系，还是相机坐标系
    ...
    # assert not torch.isnan(roughness).any(), "Inputs contain NaN"
    # assert not torch.isinf(roughness).any(), "Inputs contain InF"
    # assert not torch.isnan(light.specular[0][None, ...]).any(), "Inputs contain NaN"
    # assert not torch.isinf(light.specular[0][None, ...]).any(), "Inputs contain InF"
    ...
    # assert not torch.isnan(spec).any(), "Inputs contain NaN"
    # assert not torch.isinf(spec).any(), "Inputs contain InF"


    # if torch.isnan(spec).any():
    #     render_rgb = diffuse_rgb  # [1, H, W, 3]
    #     print("isnan")
    # else:
    ...
    if gamma:
        ...
        diffuse_rgb = linear_to_srgb(diffuse_rgb)
        specular_rgb = linear_to_srgb(specular_rgb)

        

    render_rgb = torch.where(mask, render_rgb, background)
    # irr = torch.where(mask, irr, background)
    # indir = render_rgb - irr
...
def pbr_shading_retinex(
    llnerf_r_albedo_n3: torch.Tensor,  # [H, W, 3]
    llnerf_v_viewdep_n1: torch.Tensor,  # [H, W, 1]
    # light: CubemapLight,
    # normals: torch.Tensor,  # [H, W, 3]
    # view_dirs: torch.Tensor,  # [H, W, 3]
    # albedo: torch.Tensor,  # [H, W, 3]
    ...
    results = {}
    render_retinex = llnerf_r_albedo_n3 * llnerf_v_viewdep_n1 #todo
    results.update(
        {
